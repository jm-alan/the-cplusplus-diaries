...
int main()
{
  ...
  const auto async_l_sort =
      [l_sort](auto to_sort, auto start, auto end, auto posStartIdx, auto posEndIdx)
  {
    return std::async(
        std::launch::async,
        l_sort,
  ...
  std::shared_future<std::vector<unsigned long long>>
      futPartTwo{std::async(std::launch::async, l_part, ints, 0, partOne, 4)},
      futPartThree{std::async(std::launch::async, l_part, ints, partOne, cap, 4)};
  futures.push_back(async_l_sort(ints, 0, futPartTwo, 0, 0));
  futures.push_back(async_l_sort(ints, futPartTwo, futPartTwo, 0, 1));
  futures.push_back(async_l_sort(ints, futPartTwo, futPartTwo, 1, 2));
  futures.push_back(async_l_sort(ints, futPartTwo, futPartTwo, 2, 3));
  futures.push_back(async_l_sort(ints, futPartTwo, partOne, 3, 0));
  futures.push_back(async_l_sort(ints, partOne, futPartThree, 0, 0));
  futures.push_back(async_l_sort(ints, futPartThree, futPartThree, 0, 1));
  futures.push_back(async_l_sort(ints, futPartThree, futPartThree, 1, 2));
  futures.push_back(async_l_sort(ints, futPartThree, futPartThree, 2, 3));
  futures.push_back(async_l_sort(ints, futPartThree, cap, 3, 0));
  for (size_t i{}; i < futures.size(); i++)
    futures[i]
        .wait();
